...
def split_train_test_clusters(measure, clu_thre, n_fold):
	# load cluster dict
	with open(measure+'_compound_cluster_dict_'+str(clu_thre), 'rb') as f:
		C_cluster_dict = pickle.load(f)
	with open(measure+'_protein_cluster_dict_'+str(clu_thre), 'rb') as f:
...
def load_data(measure, setting, clu_thre, n_fold):
	# load data
	with open('pdbbind_all_combined_input_'+measure,'rb') as f:
		...
		for fold in range(n_fold):
			...
			for ele in range(n_sample): 
				if pid_list[ele] not in P_cluster_dict.keys():
					#pid_list=np.delete(pid_list,np.where(pid_list==pid_list[ele]))
					continue
