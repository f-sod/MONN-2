...
def train_and_eval(train_data, valid_data, test_data, params, batch_size=32, num_epoch=30):
	...
	criterion2 = Masked_BCELoss() #warnings
	...
	for epoch in range(num_epoch):
		...
		scheduler.step()
		for param_group in optimizer.param_groups: #warnings
		...
		for i in range(int(len(train_data[0])/batch_size)):
			...
			writer.add_scalar("Total Loss 1/train", loss, epoch)
			writer.add_scalar("Affinity Loss cri1/train", loss_aff, epoch)
			writer.add_scalar("Pairwise Loss cri2/train", loss_pairwise, epoch)
			...
			writer.add_scalar("Total Loss/train", total_loss, epoch)
			writer.add_scalar("Affinity Loss/train", affinity_loss, epoch)
			writer.add_scalar("Pairwise Loss/train", pairwise_loss, epoch)
...
if __name__ == "__main__":
	...
	#evaluate scheme
	measure = "IC50" #sys.argv[1]  # IC50 or KIKD
	setting = "new_compound" #sys.argv[2]   # new_compound, new_protein or new_new
	clu_thre = 0.5 #float(sys.argv[3])  # 0.3, 0.4, 0.5 or 0.6
	n_epoch = 30
	n_rep = 10
	...
	for a_rep in range(n_rep):
		...
		for a_fold in range(n_fold):
			...
			suffixe_fodler="rep_"+str(a_rep)+"fold_"+str(a_fold)
	...
	print(f"std , {np.std(rep_avg_list, axis=0)}")
	print('Hyper-parameters:', [para_names[i]+':'+str(params[i]) for i in range(7)])
