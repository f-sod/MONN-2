...
#define the model
class Net(nn.Module):
	def __init__(self, init_atom_features, init_bond_features, init_word_features,kernel_size,params):
		super(Net, self).__init__() #super() can also take two parameters: the first is the subclass, and the second parameter is an object that is an instance of that subclass.
		...
		self.inner_CNN_depth = params["inner_CNN_depth"]
		self.DMA_depth = params["DMA_depth"]
		self.k_head = params["k_head"]
		...
		self.conv_first = nn.Conv1d(20, self.hidden_size1, kernel_size=self.kernel_size, padding=int((self.kernel_size-1)/2)) #RETURN A FLOAT 
		...
		#DMA parameters
		self.mc0 = nn.Linear(self.hidden_size2, self.hidden_size2)
		self.mp0 = nn.Linear(self.hidden_size2, self.hidden_size2)
